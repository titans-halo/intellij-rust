...
class InvertIfIntention : RsElementBaseIntentionAction<InvertIfIntention.Context>() {
    ...
    private fun ContextWithoutElse.canApplyFix(): Boolean {
        ...
        val hasImplicitReturnOrContinue = parent is RsFunctionOrLambda || parent.isLoop()
...
private fun IfStmts.addImplicitReturnOrContinue(block: RsBlock, factory: RsPsiFactory): IfStmts {
    ...
    return when {
        ...
        expr == null -> {
            ...
            val addedStmt = when {
                block.parent is RsFunctionOrLambda -> factory.createStatement("return;")
                block.parent.isLoop() -> factory.createStatement("continue;")
                else -> return this
            }
...
private fun removeLastReturnOrContinue(stmts: List<PsiElement>, block: RsBlock): List<PsiElement> {
    ...
    val hasImplicitContinue = block.parent.isLoop() && lastExpr is RsContExpr
...
private fun PsiElement.isLoop(): Boolean =
